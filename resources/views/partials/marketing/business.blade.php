            ...
            @foreach(($businesses ?? []) as $index => $business)
                <div class="{{ $index === 2 ? 'md:col-span-2' : '' }} relative group overflow-hidden rounded-2xl aspect-[16/10] {{ $index === 2 ? 'md:aspect-[21/9]' : '' }}"
                    ...
                    <div class="absolute inset-0 p-8 flex flex-col justify-between">
                        <div>
                            {{-- Dynamic Logo: Specific Business Logo OR Global Company Logo --}}
                            @php
                        ...
                        <div class="max-w-xl">
                            ...
                            <a href="{{ $business->website_link ?: route('business.show', $business->slug) }}" class="inline-flex items-center text-white border border-white/50 rounded-full px-6 py-2 hover:bg-white hover:text-abe-navy transition-all duration-300">
                                Lihat Selengkapnya
