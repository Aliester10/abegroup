    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mt-10 grid md:grid-cols-2 lg:grid-cols-2 gap-6 justify-center mx-auto">
            ...
            <a href="{{ $business->website_link ?: '#' }}"
                ...
                <div class="mb-4">
                    <img src="{{ asset('storage/' . $business->image) }}" alt="{{ $business->name }}" class="w-16 h-16 mx-auto">
                </div>
                <h3 class="text-lg font-semibold text-gray-900 group-hover:text-white text-center">{{ $business->name }}</h3>
                <p class="mt-2 text-sm text-gray-600 group-hover:text-white/70 text-center">{{ Str::limit($business->description, 140) }}</p>
            </a>
            @empty
