...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-lg shadow p-6">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit About Section</h1>
        ...
        <form action="{{ route('admin.about.update', $aboutSection) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $aboutSection->title) }}" class="w-full rounded-lg border-gray-300" required>
                ...
                <div>
                    ...
                    <input type="text" name="subtitle" value="{{ old('subtitle', $aboutSection->subtitle) }}" class="w-full rounded-lg border-gray-300">
                    @error('subtitle')<div class="text-sm text-red-600 mt-1">{{ $message }}</div>@enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Content</label>
                    <textarea name="content" rows="6" class="w-full rounded-lg border-gray-300" required>{{ old('content', $aboutSection->content) }}</textarea>
                ...
                <div>
                    ...
                    @if($aboutSection->image)
                        <div class="mt-3">
                            <img src="{{ asset('storage/' . $aboutSection->image) }}" class="h-20 w-20 object-cover rounded" alt="{{ $aboutSection->title }}">
                        </div>
                    @endif
                </div>
                ...
                <div class="grid sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="order" value="{{ old('order', $aboutSection->order) }}" class="w-full rounded-lg border-gray-300">
                        @error('order')<div class="text-sm text-red-600 mt-1">{{ $message }}</div>@enderror
                    </div>

                    <div class="flex items-center gap-3 pt-7">
                        <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300" {{ old('is_active', $aboutSection->is_active) ? 'checked' : '' }}>
