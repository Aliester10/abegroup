...
@section('title', 'Edit Home Statistic')
...
<div class="px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        <div class="flex items-center mb-2">
            ...
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Edit Home Statistic</h1>
        </div>
        <p class="text-gray-600 mt-2">Update the statistic details</p>
    </div>

    <!-- Current Statistic Preview -->
    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
        <div class="flex items-center">
            ...
            <div>
                <p class="text-sm font-medium text-blue-900">Current: {{ $homeStat->label }}</p>
                <p class="text-sm text-blue-700">Value: {{ $homeStat->value }}{{ $homeStat->suffix ?? '' }}</p>
            </div>
        </div>
    </div>

    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <form action="{{ route('admin.home-stats.update', $homeStat) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" 
                               id="suffix" 
                               name="suffix" 
                               value="{{ old('suffix', $homeStat->suffix) }}"
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors"
                               placeholder="e.g., M, +">
                        @error('suffix')
                            <p class="mt-2 text-sm text-red-600 flex items-center">
            ...
            <div class="px-6 py-6 border-b border-gray-200">
                ...
                <div>
                    ...
                    <textarea id="icon" 
                              ...
                              placeholder="e.g., &lt;svg class=&quot;w-5 h-5&quot; viewBox=&quot;0 0 20 20&quot; fill=&quot;currentColor&quot;&gt;...&lt;/svg&gt;">{{ old('icon', $homeStat->icon) }}</textarea>
                    @error('icon')
                        <p class="mt-2 text-sm text-red-600 flex items-center">
                            <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
