...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Mitra</h1>
    <p class="text-gray-600 dark:text-gray-400 mt-1">Perbarui informasi partner</p>
</div>
    ...
    <form action="{{ route('admin.partner.update', $partner) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="p-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" id="name" name="name" required value="{{ old('name', $partner->name) }}"
            ...
            <div>
                ...
                @if($partner->logo)
                    <div class="mt-4 p-4 border rounded-lg bg-gray-50 flex items-center justify-center">
                        <img src="{{ asset('storage/' . $partner->logo) }}" alt="Current Logo" class="h-16 object-contain">
                    </div>
                @endif
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Kosongkan jika tidak ingin mengubah logo. (Max: 2MB)</p>
            </div>

            <div>
                <label for="website_url" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Website URL (Opsional)</label>
                <input type="url" id="website_url" name="website_url" value="{{ old('website_url', $partner->website_url) }}"
                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" id="order" name="order" value="{{ old('order', $partner->order) }}"
                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                </div>
                <div class="flex items-center mt-6">
                    <input id="is_active" name="is_active" type="checkbox" value="1" {{ $partner->is_active ? 'checked' : '' }}
