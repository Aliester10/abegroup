...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('admin.activity.create') }}" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg transition duration-200">
            Add New Activity
        </a>
    </div>
                ...
                @forelse($activities as $activity)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            @if($activity->image)
                                <img src="{{ asset('storage/' . $activity->image) }}" alt="{{ $activity->title }}" class="h-10 w-10 object-cover rounded">
                            @else
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <a href="{{ route('admin.activity.edit', $activity) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</a>
                            <form action="{{ route('admin.activity.destroy', $activity) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-6 py-4 text-center text-gray-500">
                            No activities found. <a href="{{ route('admin.activity.create') }}" class="text-blue-500 hover:underline">Create one</a>
                        </td>
                    </tr>
                @endforelse
            </tbody>
