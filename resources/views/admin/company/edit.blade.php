        ...
        <form action="{{ route('admin.company.update', $company) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    @if($company->logo)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $company->logo) }}" alt="{{ $company->name }}" class="h-16 w-auto max-w-full object-contain">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Supported formats: JPEG, PNG, JPG, GIF, SVG. Max size: 2MB. Leave empty to keep current logo.</p>
                ...
                <div>
                    ...
                    <input type="text" name="slug" value="{{ old('slug', $company->slug) }}" class="w-full rounded-lg border-gray-300">
                ...
                <div>
                    ...
                    <input type="url" name="website_url" value="{{ old('website_url', $company->website_url) }}" class="w-full rounded-lg border-gray-300" placeholder="https://">
                    @error('website_url')<div class="text-sm text-red-600 mt-1">{{ $message }}</div>@enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Description (optional)</label>
                    <textarea name="description" rows="4" class="w-full rounded-lg border-gray-300">{{ old('description', $company->description) }}</textarea>
                ...
                <div class="grid sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="order" value="{{ old('order', $company->order) }}" class="w-full rounded-lg border-gray-300">
                        @error('order')<div class="text-sm text-red-600 mt-1">{{ $message }}</div>@enderror
                    </div>

                    <div class="flex items-center gap-3 pt-7">
                        <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300" {{ old('is_active', $company->is_active) ? 'checked' : '' }}>
                        <span class="text-sm text-gray-700">Active</span>
                    </div>
                </div>

                <div class="flex items-center gap-3">
                    <a href="{{ route('admin.company') }}" class="px-4 py-2 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-800">Cancel</a>
