                ...
                @forelse($news as $item)
                    <article class="group rounded-2xl border border-slate-200 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition">
                        ...
                        <div class="p-6">
                            ...
                            <p class="mt-2 text-sm text-slate-600">{{ Str::limit($item->excerpt ?? $item->content, 120) }}</p>
                            <div class="mt-4">
                                <a href="{{ route('news.show', $item->slug) }}" class="inline-flex items-center gap-2 text-orange-600 hover:text-orange-500 font-semibold">
                                    Baca Selengkapnya
