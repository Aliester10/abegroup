...
@section('content')
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-[2rem] shadow-2xl shadow-slate-200/60 overflow-hidden border border-slate-100">
                <div class="flex flex-col lg:flex-row min-h-[450px]">
                    ...
                    <!-- Left: Image -->
                    <div class="lg:w-1/2 relative bg-slate-900 min-h-[300px] lg:min-h-full">
                        @if($latestNews->image)
                            <img 
                        ...
                        @else
                            <div class="absolute inset-0 bg-slate-900 grid place-items-center">
                                <span class="text-slate-800 font-black text-6xl italic opacity-20 transform -rotate-12">ABE</span>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="lg:w-1/2 p-8 sm:p-12 lg:p-16 flex flex-col justify-center">
                        ...
                        <div class="flex items-center gap-4 mb-6 flex-wrap">
                            <span class="bg-orange-50 text-orange-600 text-[10px] font-bold px-3 py-1 rounded-md uppercase tracking-widest border border-orange-100">
                                Teknologi
                            </span>
                            ...
                            <div class="flex items-center gap-1.5 text-xs font-bold text-slate-400">
                                ...
                                <span>{{ $latestNews->created_at->format('d F Y') }}</span>
                            </div>
                        ...
                        <!-- Title -->
                        <h2 class="text-3xl sm:text-4xl lg:text-5xl font-black text-slate-900 leading-[1.1] mb-6">
                            {{ $latestNews->title }}
                        </h2>

                        <!-- Excerpt -->
                        <p class="text-slate-500 text-lg leading-relaxed mb-10 line-clamp-3">
                            {{ Str::limit($latestNews->excerpt ?? $latestNews->content, 180) }}
                        </p>

                        <!-- Action -->
                        <div>
                            <a href="{{ route('news.show', $latestNews->slug) }}"
                               class="inline-flex items-center gap-3 bg-[#FF6900] hover:bg-slate-900 text-white text-sm font-bold px-8 py-4 rounded-xl transition-all duration-300 shadow-xl shadow-orange-500/20">
                                Baca Selengkapnya
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <form action="{{ route('news') }}" method="GET">
                    <div class="relative flex items-center">
                        <input 
                            ...
                            class="w-full pl-14 pr-6 py-4 text-slate-900 bg-white border border-slate-200 rounded-2xl shadow-xl shadow-slate-200/40 focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all duration-300">
                        <div class="absolute inset-y-0 left-5 flex items-center pointer-events-none">
                            <svg class="h-6 w-6 text-slate-400 group-focus-within:text-orange-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <section class="pb-24">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <!-- Title -->
            <h2 class="text-3xl sm:text-4xl font-black text-slate-900 tracking-tight mb-8">Semua <span class="text-slate-900">Berita</span></h2>

            <!-- Category Tabs -->
            <div class="mb-10 border-b border-slate-200">
                <nav class="flex flex-wrap gap-x-1 -mb-px" aria-label="Kategori Berita">
                    <a href="{{ route('news', array_merge(request()->only('search'), [])) }}" 
                       class="px-4 py-3 text-sm font-semibold transition-colors duration-200 border-b-2 whitespace-nowrap {{ !$category ? 'text-orange-600 border-orange-500' : 'text-slate-500 border-transparent hover:text-slate-700 hover:border-slate-300' }}">
                        Semua
                    </a>
                    @foreach($categories as $key => $label)
                        <a href="{{ route('news', array_merge(request()->only('search'), ['category' => $key])) }}" 
                           class="px-4 py-3 text-sm font-semibold transition-colors duration-200 border-b-2 whitespace-nowrap {{ $category == $key ? 'text-orange-600 border-orange-500' : 'text-slate-500 border-transparent hover:text-slate-700 hover:border-slate-300' }}">
                            {{ $label }}
                        </a>
                    @endforeach
                </nav>
                ...
                @forelse($otherNews as $item)
                    <article class="group bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-400 flex flex-col h-full overflow-hidden">
                        ...
                        <div class="p-5 sm:p-6 flex flex-col flex-1">
                            ...
                            <!-- Judul -->
                            <h3 class="text-base sm:text-lg font-bold text-slate-900 leading-snug mb-4 line-clamp-2 group-hover:text-slate-700 transition-colors duration-200">
                            ...
                            <div class="mt-auto">
                                <a href="{{ route('news.show', $item->slug) }}" class="inline-flex items-center gap-1.5 text-orange-600 hover:text-orange-700 font-bold text-sm transition-colors duration-200">
                                    Baca Selengkapnya
                                    <svg class="w-3.5 h-3.5 group-hover:translate-x-0.5 transition-transform duration-200" viewBox="0 0 20 20" fill="currentColor">
